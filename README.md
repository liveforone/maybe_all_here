...
## 시큐리티 문제
* 시큐리티가 종종 문제를 일으킨다.
* 유저 서비스에서는 시큐리티가 authorization header filter의 역할을 이미 하고 있으므로 gateway에서는 필터를 걸지 않는다.
* 또한 시큐리티 url 설정시에는 외부에 제공하는 api를 permitall로 설정해야한다. 
* 그렇지 않으면 403에러가 계속해서 뜬다.(토큰을 넣었어도)
* 다른 서비스들은 게이트웨이에서 주관하기 때문에 권한에 큰 문제가 없지만 시큐리티가 권한을 주관하는 유저서비스는 외부 제공 api에 반드시 permitAll로 설정한다.
